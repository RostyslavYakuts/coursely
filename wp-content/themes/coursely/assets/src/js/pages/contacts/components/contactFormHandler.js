...
export const contactFormHandler = () => {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        if (name.length < 2) {
            ...
            nameLabel.classList.add('label-error');
            nameError.textContent = "Text is too long (max. 200 chars)";
            nameError.classList.remove("hidden");
            form.querySelector("#user_name").classList.add("border-error");
            valid = false;
        }else{
        ...
        try {
            ...
            if (result.success === false) {
                globalMsg.innerHTML = result?.data?.message || "Error. Try again latter.";
            ...
            globalMsg.innerHTML = result?.data?.message || "Thanks for your message";
            showThankYouModal('success');
        } catch (error) {
            console.log(error);
            globalMsg.textContent = "Connecting error. Try again latter";
            showThankYouModal('fail');
        }
    });
};
