...
<div class="summary px-10 pt-5 pb-10 w-full rounded-[20px] bg-[#1C55E01A] ">
    <h2 class="mt-5 font-bold text-[32px]">{{__('Summary','coursely')}}</h2>
    <div class="mt-6 px-6 py-8 rounded-[20px] bg-white brand-shadow">
        <h3 class="p-0 m-0 font-bold text-[24px]">{{$data['plan']['duration']}}</h3>
        <p class="mt-4 text-brand-text text-lg">{{__('Stay consistent with your','coursely')}}</p>

        <div class="mt-8">
            <span class="font-bold text-[36px]">${{$data['plan']['price']}}</span>
            <span class="text-brand-text">/ {{$data['plan']['duration']}}</span>
        </div>
    ...
    <div class="agreement-info py-2.5 mt-6 font-medium flex flex-col gap-6">
        <span>{{__('Recurring billing every','coursely')}} {{$data['plan']['duration']}}</span>
        <span>
                    By clicking “Pay Now”, you agree to our <a class="underline" href="{{home_url('terms-of-service')}}">Terms of Service</a> and <a class="underline" href="{{home_url('privacy-policy')}}">Privacy Policy</a>
                </span>
        <span>
                    {{__('Payments are charged in USD. Payment provider fees may apply','coursely')}}
                </span>
    </div>
</div>