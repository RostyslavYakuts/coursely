{{-- Global Pricing --}}

@php

    use coursely\App\Core\Services\SubscriptionManager;

    $current_user_id = get_current_user_id();
    $active_plan = SubscriptionManager::getActivePlan($current_user_id);

    $feature_svg = '<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M9 0C7.21997 0 5.47991 0.527841 3.99987 1.51677C2.51983 2.50571 1.36628 3.91131 0.685088 5.55585C0.00389957 7.20038 -0.17433 9.00998 0.172937 10.7558C0.520203 12.5016 1.37737 14.1053 2.63604 15.364C3.89472 16.6226 5.49836 17.4798 7.24419 17.8271C8.99002 18.1743 10.7996 17.9961 12.4442 17.3149C14.0887 16.6337 15.4943 15.4802 16.4832 14.0001C17.4722 12.5201 18 10.78 18 9C18 6.61305 17.0518 4.32387 15.364 2.63604C13.6761 0.948212 11.387 0 9 0ZM13.23 6.46L8.79 12.62C8.65775 12.7985 8.46474 12.9225 8.24737 12.9686C8.03001 13.0146 7.8033 12.9796 7.61 12.87L5.06 11.38C4.85314 11.2553 4.70426 11.0536 4.64612 10.8192C4.58798 10.5848 4.62535 10.3369 4.75 10.13C4.87466 9.92313 5.07638 9.77425 5.3108 9.71612C5.54522 9.65798 5.79314 9.69535 6 9.82L7.81 10.9L11.81 5.41C11.9585 5.26128 12.1543 5.16919 12.3635 5.1497C12.5727 5.13021 12.7822 5.18454 12.9556 5.30329C13.1289 5.42204 13.2553 5.5977 13.3127 5.79984C13.3701 6.00198 13.355 6.21784 13.27 6.41L13.23 6.46Z" fill="#1C55E0"/>
    </svg>';
    $feature_svg_popular = '<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M9 0C7.21997 0 5.47991 0.527841 3.99987 1.51677C2.51983 2.50571 1.36628 3.91131 0.685088 5.55585C0.00389957 7.20038 -0.17433 9.00998 0.172937 10.7558C0.520203 12.5016 1.37737 14.1053 2.63604 15.364C3.89472 16.6226 5.49836 17.4798 7.24419 17.8271C8.99002 18.1743 10.7996 17.9961 12.4442 17.3149C14.0887 16.6337 15.4943 15.4802 16.4832 14.0001C17.4722 12.5201 18 10.78 18 9C18 6.61305 17.0518 4.32387 15.364 2.63604C13.6761 0.948212 11.387 0 9 0ZM13.23 6.46L8.79 12.62C8.65775 12.7985 8.46474 12.9225 8.24737 12.9686C8.03001 13.0146 7.8033 12.9796 7.61 12.87L5.06 11.38C4.85314 11.2553 4.70426 11.0536 4.64612 10.8192C4.58798 10.5848 4.62535 10.3369 4.75 10.13C4.87466 9.92313 5.07638 9.77425 5.3108 9.71612C5.54522 9.65798 5.79314 9.69535 6 9.82L7.81 10.9L11.81 5.41C11.9585 5.26128 12.1543 5.16919 12.3635 5.1497C12.5727 5.13021 12.7822 5.18454 12.9556 5.30329C13.1289 5.42204 13.2553 5.5977 13.3127 5.79984C13.3701 6.00198 13.355 6.21784 13.27 6.41L13.23 6.46Z" fill="white"/>
    </svg>';
@endphp

@if($active_plan && $data['plans'])
    @php
        $stripe_price_id = $active_plan->stripe_price_id;
    @endphp
    <div class="container">
        <div class="plans-wrapper mt-16 grid grid-cols-1 md:grid-cols-2 lgx:grid-cols-4 gap-5">
            @foreach($data['plans'] as $plan)
                <div class="brand-shadow plan bg-white rounded-[20px] py-8 px-6 w-full flex flex-col gap-5"
                     data-type="{{$plan['type']}}">

                    @if($plan['stripe_price_id'] === $stripe_price_id)
                    <div class="flex justify-between items-center gap-2">
                         <span class="font-bold text-[20px] lgx:text-[24px]">
                            {{$plan['duration']}}
                        </span>
                        <span class="font-medium text-xs rounded-[8px] bg-brand text-white py-1.5 px-2">
                            {{__('Current plan','coursely')}}
                        </span>
                    </div>
                    @else
                        <span class="font-bold text-[20px] lgx:text-[24px]">
                            {{$plan['duration']}}
                        </span>
                    @endif


                    <span class="text-brand-text lgx:text-lg">
                        {{$plan['description']}}
                    </span>

                    <span class="highlighted-text font-bold text-brand">
                        {{$plan['highlighted_text']}}
                    </span>

                    <span class="flex flex-row items-end gap-1">
                        <span class="font-bold text-[32px] lgx:text-[36px] leading-none">
                            {{$plan['price']}}
                        </span>
                        <span class="text-brand-text">/{{$plan['per_period']}}</span>
                    </span>

                    <a href="{{get_home_url()}}/checkout?price_id={{$plan['stripe_price_id']}}"
                       data-plan-type="{{$plan['type']}}"
                       class="bg-white flex justify-center items-center gap-2 p-3 w-full rounded-full border border-brand-gray text-lg hover:text-white hover:bg-brand-dark brand-btn-light">
                        {{$data['plans_cta']}}
                        <svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9.16667 1L15 7M15 7L9.16667 13M15 7L1 7" stroke="#111230" stroke-width="2"
                                  stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </a>

                    <div class="w-full h-[1px] bg-brand-gray"></div>

                    <span class="font-bold lgx:text-lg">
                        {{$data['plans_features_text']}}
                    </span>

                    @if($plan['features'])
                        @foreach($plan['features'] as $feature)
                            <div class="flex flex-row gap-3 items-center text-brand-text">
                                @if($plan['is_popular'])
                                    {!! $feature_svg_popular !!}
                                @else
                                    {!! $feature_svg !!}
                                @endif
                                <span>{{$feature['feature']}}</span>
                            </div>

                        @endforeach
                    @endif

                </div>
            @endforeach
        </div>
    </div>
@endif
@if($data['plans'] && !$active_plan)
    <div class="container">
        <div class="plans-wrapper mt-16 grid grid-cols-1 md:grid-cols-2 lgx:grid-cols-4 gap-5">
            @foreach($data['plans'] as $plan)
                <div class="brand-shadow plan {{ $plan['is_popular'] ? 'bg-brand-dark lgx:-mt-5 lgx:mb-5' :'bg-white' }} rounded-[20px] py-8 px-6 w-full flex flex-col gap-5 {{$plan['is_popular'] ?? 'popular'}}"
                     data-type="{{$plan['type']}}">

                    <span class="font-bold text-[20px] lgx:text-[24px] {{ $plan['is_popular'] ? 'text-white' :'text-brand-dark' }}">
                        {{$plan['duration']}}
                    </span>

                    <span class="{{ $plan['is_popular'] ? 'text-white' :'text-brand-text' }} lgx:text-lg">
                        {{$plan['description']}}
                    </span>

                    <span class="highlighted-text font-bold {{ $plan['is_popular'] ? 'text-white' :'text-brand' }}">
                        {{$plan['highlighted_text']}}
                    </span>

                    <span class="flex flex-row items-end gap-1">
                        <span class="{{ $plan['is_popular'] ? 'text-white' :'text-brand-dark' }} font-bold text-[32px] lgx:text-[36px] leading-none">
                            {{$plan['price']}}
                        </span>
                        <span class="{{ $plan['is_popular'] ? 'text-white' :'text-brand-text' }} ">/{{$plan['per_period']}}</span>
                    </span>

                    <a href="{{get_home_url()}}/checkout?price_id={{$plan['stripe_price_id']}}"
                       data-plan-type="{{$plan['type']}}"
                       class="bg-white flex justify-center items-center gap-2 p-3 w-full rounded-full border border-brand-gray text-lg hover:text-white hover:bg-brand-dark brand-btn-light">
                        {{$data['plans_cta']}}
                        <svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9.16667 1L15 7M15 7L9.16667 13M15 7L1 7" stroke="#111230" stroke-width="2"
                                  stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </a>

                    <div class="w-full h-[1px] bg-brand-gray"></div>

                    <span class="font-bold lgx:text-lg {{ $plan['is_popular'] ? 'text-white' :'text-brand-dark' }}">
                        {{$data['plans_features_text']}}
                    </span>

                    @if($plan['features'])
                        @foreach($plan['features'] as $feature)
                            <div class="flex flex-row gap-3 items-center {{ $plan['is_popular'] ? 'text-white' :'text-brand-text' }}">
                                @if($plan['is_popular'])
                                    {!! $feature_svg_popular !!}
                                @else
                                    {!! $feature_svg !!}
                                @endif
                                <span>{{$feature['feature']}}</span>
                            </div>

                        @endforeach
                    @endif

                </div>
            @endforeach
        </div>
    </div>
@endif
