    ...
    <div class="container">
        <div class="plans-wrapper mt-16 grid grid-cols-1 md:grid-cols-2 lgx:grid-cols-4 gap-5">
            @foreach($data['plans'] as $plan)
                <div class="brand-shadow plan {{ $plan['is_popular'] ? 'bg-brand-dark lgx:-mt-5 lgx:mb-5' :'bg-white' }} rounded-[20px] py-8 px-6 w-full flex flex-col gap-5 {{$plan['is_popular'] ?? 'popular'}}" data-type="{{$plan['type']}}">
                    ...
                    <a href="{{get_home_url()}}/checkout?price_id={{$plan['stripe_price_id']}}" data-plan-type="{{$plan['type']}}" class="bg-white flex justify-center items-center gap-2 p-3 w-full rounded-full border border-brand-gray text-lg hover:text-white hover:bg-brand-dark brand-btn-light">
                        {{$data['plans_cta']}}
