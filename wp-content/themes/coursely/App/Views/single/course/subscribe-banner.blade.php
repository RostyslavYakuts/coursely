...
<div class="active-subscription w-full lgx:w-[406px] lgx:min-w-[406px] lgx:h-[365px] mt-14 bg-white rounded-[20px] brand-shadow p-5 lgx:py-8 lgx:px-6 flex flex-col">
    <h3 class="leading-none font-bold text-[24px]">{{__('Click to Start learning now','coursely')}}</h3>
    <div class="mt-8 flex bg-[#EBFAF5] rounded-[10px] p-3">
        <span class="text-lg">
            {{__('This course is already included in your subscription.','coursely')}}
        </span>
    </div>

    @include('global.course-progress')

    <a href="{{$data['cta_link']}}" class="mt-8 mx-auto w-full flex items-center justify-center brand-btn-dark">
        {{$data['cta_text']}}
    </a>
</div>
@else
